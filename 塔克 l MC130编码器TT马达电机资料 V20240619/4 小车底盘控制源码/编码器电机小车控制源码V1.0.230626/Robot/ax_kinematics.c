...
//函数定义
void ROBOT_MEC_Kinematics(void);   //麦轮模型
void ROBOT_FWD_Kinematics(void);   //四轮差速模型
void ROBOT_TWD_Kinematics(void);   //两轮差速模型
void ROBOT_AKM_Kinematics(void);   //阿克曼模型
void ROBOT_OMT_Kinematics(void);   //三轮全向模型

void AX_ROBOT_Stop(void);   //机器人停止运动函数
...
{
	double akm_spd;  //速度
    double akm_angle; //前轮转向角度
	double akm_angle_r;  //右前轮转向角度
	double akm_angle_s;  //舵机转向角度
	double radius;  //转弯半径
