...
/******************************************************************************
例程名称：正交AB编码器例程
例程说明：200ms电机编码器数值并串口输出显示
操作说明：电机连接后，即可手动转动电机观察串口输出值变化
*******************************************************************************/
int main(void)
{
	uint16_t temp;
  ...
  AX_LED_Init();  
	...
	//OLED初始化及配置
	AX_OLED_Init();
	AX_OLED_DisplayAsciiStr(0, 0, "ENCODER TEST", 0);
	AX_OLED_DisplayAsciiStr(0, 16, "NUM:", 0);	
  ...
  AX_ENCODER_Init(60000);  
	...
	while (1) 
	{		
		temp = AX_ENCODER_GetCounter();
		printf("*ENCODER:%5d \r\n",temp);  
		AX_OLED_DisplayValue(24, 16, temp, 5, 0, 0);  //显示数值
		AX_OLED_RefreshScreen();			
		AX_Delayms(200);
	}
}	
